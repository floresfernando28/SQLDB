/*----------------------------------------------------------------------------
 ||            Creador de Base de Datos Relacional:						     ||
 ||																			 ||
 ||         -Fernando José Flores Mendoza								     ||
...
if db_id ('Sistema_Supermercado') is not null
begin
   use master
   drop Database Sistema_Supermercado
end


/*------------------------------------------------------------------------------

 Creamos la Base de datos con la que vamos a trabajar, ya en la condición anterior
 determinamos si la base de datos existe, si existio fue eliminada y asi la podemos 
 crear nosotros.

  1-Datos primarios tienen la extención (.mdf) contiene todos los archivos
    de mi base de datos (ARCHIVO PRINCIPAL DE LA BASE DE DATOS).
  2-Datos secundarios tiene la extención (.ndf). 
  3-Datos Log tienen la extención (.ldf) se guardan todas las transacciones 
    de mi base de datos

	NOTA: Estos archivos deben guardarse en una carpeta dentro del disco local C
	      en mi caso la guarde en una carpeta llamada (C:\Sistema_Supermercado)
		  para ejecutar este proceso a continuación deben primero crear la carpeta
		  con este nombre (C:\Sistema_Supermercado) y en el disco local.
		 -El nombre puede ser otro, pero tambien debera ser modificado en los
		  archivos o rutas que pusimos con la extención (.mdf)(.ndf)(.ldf) 
		  a continuación
*/

Create Database Sistema_Supermercado
on primary (
	name = 'Sistema_Supermercado_mdf',
	filename = 'C:\Sistema_Supermercado\Sistema_Supermercado.mdf',
	size = 15mb,
	maxsize = 250mb,
	filegrowth = 10mb
),(
	name = 'Sistema_Supermercado_ndf',
	filename = 'C:\Sistema_Supermercado\Sistema_Supermercado.ndf',
	size = 15mb,
	maxsize = unlimited,
	filegrowth = 10mb
)
log on (
	name = 'Sistema_Supermercado_ldf',
	filename = 'C:\Sistema_Supermercado\Sistema_Supermercado.ldf',
	size = 15mb,
	maxsize = 500mb,
	filegrowth = 10%
)
...
/*-------------------------------------------------------------------------------
 Ya tenemos creada la base de datos (Sistema_Supermercado)
 pero aun no la podemos utilizar porque en la condición anterior asignamos
 por defecto a la base de datos (master) y entonces debemos usar la base de 
 datos que nosotros creamos con la siguiente sentencia a continuacion:
*/
...
insert into usuario (Nombre_Usuario, contrasenia) values ('norman6', 'elbicho');
...
values ('Fernando Jose', 'Flores Mendoza', 19, '20020302', null ,1, 'M');

insert into Empleados (Nombre_Empleado, Apellido_Empleado, Edad, Fecha_Nacimiento, Id_Cargo, Id_Usuario, sexo)
values ('Brayan Ernesto', 'Flores Mendoza', 13, '20080707', null ,2, 'M');

insert into Empleados (Nombre_Empleado, Apellido_Empleado, Edad, Fecha_Nacimiento, Id_Cargo, Id_Usuario, sexo)
values ('Brandon Jose', 'Lopez Ruiz', 18, '20030504', null ,3, 'M');
